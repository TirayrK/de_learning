...
try:
    ...
    if null_sentiments > row_count * 0.1:
        logger.warning(f"{null_sentiments:,} rows ({null_sentiments / row_count * 100:.1f}%) have null sentiment scores")
...
try:
    ...
    logger.info(f"Aggregated {unique_products:,} unique products (avg {row_count / unique_products:.1f} reviews/product)")
